...
static bool dupmap_less(const struct hash_elem *a, const struct hash_elem *b,
                        void *aux) {
...
static void initd(void *aux_) {
  ...
  if (cur->fd_table == NULL || cur->fd_cap == 0) {
    ...
    if (cur->fd_table == NULL) PANIC("fd_table alloc failed");
...
int process_exec(void *f_name) {
  ...
  for (char *p = strtok_r(NULL, " ", &save_ptr); p != NULL && argc < 63;
  ...
  size_t pad =
      ((top - str_bytes - pushes) & 0xfull);  // 0..15, 최종 RSP 16B 정렬 보정
...
int process_wait(tid_t child_tid UNUSED) {
   ...
   * XXX:       process_wait를 구현하기 전에는 이곳에 무한 루프를 넣는 것을
   * 권장한다. */
  struct thread *cur = thread_current();
  struct list_elem *e;
  ...
  for (e = list_begin(&cur->children); e != list_end(&cur->children);
    ...
    if (cs->tid == child_tid) {
      ...
      if (!cs->exited) sema_down(&cs->sema);
...
void process_exit(void) {
  ...
  if (cur->proc_inited) {
    ...
    if (cur->my_status) {
      ...
      if (--cur->my_status->ref_cnt == 0) free(cur->my_status);
      cur->my_status = NULL;
    }
