...
/*
  ...
  DESTRUCTOR가 NULL이 아니면, 해시의 각 요소에 대해 호출됩니다. DESTRUCTOR는
  적절한 경우 해시 요소가 사용한 메모리를 해제할 수 있습니다. 그러나
  hash_clear()가 실행되는 동안 해시 테이블 H를 수정하는 것(hash_clear(),
  hash_destroy(), hash_insert(), hash_replace(), hash_delete() 함수 사용)은
  DESTRUCTOR 내부든 외부든 정의되지 않은 동작(undefined behavior)을
  발생시킵니다.
*/
...
/*
  해시 테이블 H를 파괴합니다.
  DESTRUCTOR가 NULL이 아니면, 해시의 각 요소(element)에 대해 먼저 호출됩니다.
  DESTRUCTOR는 적절한 경우 해시 요소가 사용한 메모리를 해제할 수 있습니다.
  그러나 hash_clear()가 실행되는 동안 해시 테이블 H를 수정하는 것(hash_clear(),
  hash_destroy(), hash_insert(), hash_replace(), hash_delete() 함수 사용)은
  DESTRUCTOR 내부든 외부든 정의되지 않은 동작(undefined behavior)을
  발생시킵니다.
 */
